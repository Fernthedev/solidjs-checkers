...
import { MultiplayerCore, Player } from "./multiplayer";
...
export class LocalMultiplayer implements MultiplayerCore {
    ...
    getPieces(player?: Player | undefined): Readonly<Record<number, CheckerboardPiece>> {
        if (!player) return this.pieces

        return Object.fromEntries(Object.values(this.pieces).filter(p => p.player === player).map(e => [e!.position, e]))
    }
    ...
    takeTurn(piece: CheckerboardPiece, square: number): void {
        ...
        if (!Array.from(calculatePlayableSpots(piece, this.width, this.height, piecesValues)).some(e => e === square))
    ...
    playablePositions(piece: CheckerboardPiece): Generator<number> {
        return calculatePlayableSpots(piece, this.width, this.height, Object.values(this.pieces))
    }
